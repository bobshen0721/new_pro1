...
GitHub Release 僅提供三個可公開取得的模型，並以未壓縮 `tar` 封裝後切成小於 2 GiB 的分片。`restore_release_models.ps1` 會逐片驗證 SHA-256、重組封裝、再次驗證完整封裝，最後解壓到本機 `models` 資料夾。

## 收錄模型

| 本機資料夾 | 原始來源 | 授權 |
|---|---|---|
| `faster-whisper-large-v2` | [Systran/faster-whisper-large-v2](https://huggingface.co/Systran/faster-whisper-large-v2) | MIT |
| `TEA-ASR-1.1` | [JacobLinCool/TEA-ASR-1.1](https://huggingface.co/JacobLinCool/TEA-ASR-1.1) | TEA checkpoint：MIT；底層 Qwen 權重仍受 Apache-2.0 與 NOTICE／歸屬要求約束 |
